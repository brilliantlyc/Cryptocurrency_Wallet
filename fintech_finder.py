...
# * Generate a new Ethereum account instance by using our mnemonic seed phrase
# (which we created earlier in the module).

# * Fetch and display the account balance associated with our Ethereum account
# address.
...
# Once we receive the transaction’s hash code, we will navigate to the Transactions
# section of Ganache to review the blockchain transaction details. To confirm that 
# we have successfully created the transaction, we will save screenshots to the 
# README.md file of our GitHub repository for this Challenge assignment.
...
# In this section, we'll import several functions from the `crypto_wallet.py`
...
# application. For this section, we will assume the perspective of a Fintech
# Finder customer (i.e., we’ll provide our Ethereum wallet and account
# information to the application).

# Complete the following steps:

# 1. Review the code contained in the `crypto_wallet.py` script file. Note that
# the Ethereum transaction functions that we have built throughout this
# module-including `wallet`, `wallet.derive_acount`, `get_balance`, `fromWei`,
# `estimateGas`, `sendRawTransaction`, and others have now been
# incorporated into Python functions that allow us to automate the process of
# accessing them.

# 2. Add our mnemonic seed phrase (provided by Ganache) to the starter code’s `SAMPLE.env` file.
...
# case, our) HD wallet and Ethereum account.
...
# and pass it our Ethereum `account.address`.
...
# This function will create the Fintech Finder customer’s (in this case, our) HD wallet 
...
# Call `get_balance` function and pass it our account address, in addition to 'w3'
...
# which they’ll hire the worker. We code the application so that once a customer
# completes these steps, the application will calculate the amount that the
# worker will be paid in ether. To do so, we complete the following steps:
...
# that will allow a customer (us, in this case) to send an Ethereum blockchain
# transaction that pays the hired candidate. To accomplish this, locate the
# code that reads `if st.sidebar.button("Send Transaction")`. We’ll need to
# add logic to this `if` statement that sends the appropriate information to
# the `send_transaction` function (which we imported from the `crypto_wallet`
# script file). Inside the `if` statement, add the following functionality:

    # * Call the `send_transaction()` function and pass it three parameters:
        # - Our Ethereum `account` information. (Remember that this `account`
...
if st.sidebar.button("Send Transaction"):
    ...
    # Our `account`, the `candidate_address`, and the `wage` as parameters, in addition to 'w3'
...
# 1. From our terminal, we navigate to the project folder that contains
# our `.env` file and the `fintech_finder.py` and `crypto_wallet.py` files.
# Being sure to activate our Conda `dev` environment if it is not already active.

# 2. To launch the Streamlit application,
# type `streamlit run fintech_finder.py`.

# 3. On the resulting webpage, select a candidate that we would like to hire
# from the appropriate drop-down menu. Then, enter the number of hours that we
# would like to hire them for. (Remember, we do not have a lot of ether in
# our account, so we cannot hire them for long!)

# 4 Click the Send Transaction button to sign and send the transaction with
# our Ethereum account information. If the transaction is successfully
# communicated to Ganache, validated, and added to a block,
# a resulting transaction hash code will be written to the Streamlit
# application sidebar.
    
# 5. Navigate to the Ganache accounts tab and locate our account (index 0).
    # * Take a screenshot of the address, balance, and transaction (TX) count.
    # Save this screenshot to the README.md file of our GitHub repository for
